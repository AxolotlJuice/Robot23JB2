...
import Team4450.Robot23.Constants.ScoreMode;

import org.opencv.core.Rect;
import org.opencv.core.Rect2d;


public class AimBotNFO extends CommandBase {

    //kEndGoalY is temporary
    private static double           kP = .3, kI = 0.03, kD = 0.0, kP2 = .3, kI2 = 0.03, kD2 = 0.0, 
                                    kToleranceX = 0.0, kToleranceY = 0.0, kEndGoalY = 0.0;
   
    private double                  startTime, tempTime, elapsedTime, powerS, powerT;
    private double                  latestTimestamp;

    private boolean                 hadTargets, noTargets = false;
    ...
    private Pose3d                  zeroPose, zeroTargetPose;
    private Pose2d                  latestRobotPose, targetPose2d;
    ...
    public AimBotNFO(PhotonCamera phCamera,
                            DriveBase driveBase,
    ...
    public void initialize(){
        ...
        /* phPoseEstimator = new PhotonPoseEstimator(tagLayout, 
    ...
    public void execute(){
        ...
        if(scoreMode == ScoreMode.CUBE && result.hasTargets()){
            //PipelineManager pipelineManager = new PipelineManger();
            ...
            targetRotation = Constants.APRILTAGFIELDLAYOUT.getTags().get(target.getFiducialId()).pose.toPose2d().getRotation()
                            .minus(((Constants.APRILTAGFIELDLAYOUT.getTags().get(target.getFiducialId()).pose).plus(target.getBestCameraToTarget().inverse()).toPose2d()).getRotation());

            driveBase.drive(0.0, 0.0, targetRotation.getRadians());

            zeroPose = new Pose3d(new Translation3d(0.0, 0.0, 0.0), new Rotation3d(0.0, 0.0, 0.0));

            //obtains the Robots pose according to the PhotonVision
            zeroTargetPose = zeroPose.plus(target.getBestCameraToTarget());
            ...
            targetPose2d = (zeroTargetPose.toPose2d().transformBy(new Transform2d(translationTarget, new Rotation2d(0.0)))).plus(transformTarget);
            ...
            powerS = -pidS.calculate(latestRobotPose.getX(), elapsedTime);
            powerT = -pidT.calculate(latestRobotPose.getY(), elapsedTime);
        
            //drive in the drection, rotation is temp
            driveBase.drive(powerS, powerT,
                            0.0);


            Util.consoleLog("PhotonHasTargets:%b, LimeHasTargets:%b, HadTargets:%b, ScoreMode:%s, PoseX=%.2f, PoseY =%.2f, targetRotation=%.2f, Strafe=%.2f, Throttle=%.2f", 
                            result.hasTargets(), limeLight.targetVisible(), hadTargets, scoreMode.name(),
                            targetPose2d.getX(), targetPose2d.getY(), targetRotation.getRadians(), powerS, powerT);
