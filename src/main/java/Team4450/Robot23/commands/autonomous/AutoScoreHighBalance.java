 ...
 * is selected.
 */
public class AutoScoreHighBalance extends CommandBase
{
	 ...
	 */
	public AutoScoreHighBalance(DriveBase driveBase, Winch winch, Arm arm, Claw claw,
	...
	public void initialize() 
	{
		...
		// Distance to strafe in meters.

		if (startingPoseIndex == 1) strafe = 1.0;
        if (startingPoseIndex == 9) strafe = -1.0;
		...
		// Next action is to drive forward distance meters and stop.
		
		command = new AutoDriveProfiled(driveBase, distance, StopMotors.stop, Brakes.off);
		
		pCommands.addCommands(command);

		// Next action is to strafe based on starting position.

		command = new AutoStrafeProfiled(driveBase, strafe, StopMotors.stop, Brakes.off);

		commands.addCommands(pCommands);

		// Next action is to reverse based on distance onto the charging station.

		command = new AutoDriveProfiled(driveBase, -(distance - 1.5), StopMotors.stop, Brakes.off);
		
		pCommands.addCommands(command);

		// Next action is to automatically balance the robot using a PID.
